...
.shop-card.rare::after{content:'★ NADİR';position:absolute;top:7px;right:7px;font-size:9px;color:var(--gold);font-weight:700;}
.shop-card.epic{border-color:rgba(180,0,255,0.25);}
.shop-card.epic::after{content:'◆ EPİK';position:absolute;top:7px;right:7px;font-size:9px;color:#cc88ff;font-weight:700;}
...
function fix(d){
  // CRITICAL: parse coins as float each time
  if(isOwner){d.coins=999999999;}
  else{d.coins=parseFloat(d.coins)||0;}
...
function doLogin(){
  ...
  if(!un||!pw)return setMsg("Tüm alanları doldur!");
  const d=S.get(un);
  if(!d)return setMsg("Kullanıcı bulunamadı!");
  if(d.password!==btoa(pw))return setMsg("Şifre yanlış!");
  startGame(un,false);
}
function doFounderLogin(){
  const pw=document.getElementById("fPass").value;
  if(pw!==OWN_P)return setMsg("Yanlış şifre!");
...
function doRegister(){
  ...
  if(!un||!pw)return setMsg("Tüm alanları doldur!");
  if(un===OWN_U)return setMsg("Bu kullanıcı adı alınmış!");
  if(pw!==p2)return setMsg("Şifreler eşleşmiyor!");
  if(un.length<3)return setMsg("En az 3 karakter!");
  if(S.get(un))return setMsg("Bu kullanıcı adı alınmış!");
  const d=fresh();d.password=btoa(pw);d.avatar=selAv;
  S.set(un,d);setMsg("Kayıt başarılı!",true);
...
function startAutoFeed(){
  ...
  autoFeedId=setInterval(()=>{
    ...
    S.set(me,d);updateUI();notif("🌾 Otomatik yem!");
  },30000);
}
function setWx(){
...
function draw(ts){
  ...
  const base=wx.name==="Gece"?"#000508":wx.name==="Karlı"?"#051525":"#020e1f";
  const bg=cx.createLinearGradient(0,0,0,H);
  bg.addColorStop(0,base);bg.addColorStop(.55,"#031e42");bg.addColorStop(1,"#010810");
  cx.fillStyle=bg;cx.fillRect(0,0,W,H);
  if(wx.name==="Karlı"){cx.fillStyle="rgba(200,230,255,.25)";cx.fillRect(0,0,W,H);}
  ...
  const bs=wx.name==="Fırtınalı"?3:1;
...
function onCVClick(e){
  ...
  const pt=document.createElement("div");pt.className="feed-pt";pt.textContent="🌾";
  ...
  if(fed){d.qs.feedCount=(d.qs.feedCount||0)+1;S.set(me,d);syncQ(d);updateUI();notif("🌾 Balık beslendi!");}
}
...
function openProfile(){
  ...
  document.getElementById("profileBody").innerHTML=`
    ...
    <div class="profile-level">${isOwner?"👑 Kurucu":li.cur.n}</div>
    <div class="profile-stats">
      <div class="stat-box"><div class="stat-val">${isOwner?"∞":Math.floor(d.coins)}</div><div class="stat-lbl">💰 Coin</div></div>
      <div class="stat-box"><div class="stat-val">${isOwner?"MAX":d.level}</div><div class="stat-lbl">⭐ Seviye</div></div>
      <div class="stat-box"><div class="stat-val">${d.fish.length}</div><div class="stat-lbl">🐠 Balık</div></div>
      <div class="stat-box"><div class="stat-val">${d.qDone||0}</div><div class="stat-lbl">📋 Görev</div></div>
    </div>
    <div class="profile-sec">🏅 ROZETLER</div>
    <div class="profile-badges">${
      !d.badges||d.badges.length===0
        ?'<div class="no-content">Henüz rozet yok</div>'
...
function openQuests(){
  ...
  if(d.gifts&&d.gifts.length>0){
    ...
    gs.innerHTML=`<div class="section-title">🎁 YÖNETİCİ HEDİYELERİ</div>`+
      d.gifts.map((g,i)=>`
        <div class="gift-card">
          <div class="gift-icon">🎁</div>
          <div class="gift-info">
            <div class="gift-title">👑 <b>${OWN_U}</b> size hediye gönderdi!</div>
            <div class="gift-msg">${g.message}</div>
          </div>
          <button class="gift-claim-btn" onclick="claimGift(${i})">✓ Aldım!</button>
        </div>`).join("");
  ...
  Object.keys(d.quests).forEach(qid=>{
    ...
    c.innerHTML=`
      ...
      <div class="quest-info">
        ...
        ${done&&!q.claimed?`<button class="quest-claim-btn" onclick="claimQ('${qid}')">💰 +${qd.reward} Al!</button>`:""}
        ${q.claimed?`<div class="quest-done-lbl">✓ Tamamlandı</div>`:""}
      </div>
      <div class="quest-reward">💰 ${qd.reward}</div>`;
    el.appendChild(c);
  });
  ...
  document.getElementById("questTimer").textContent=`🕐 Yenilenmesine: ${h}s ${m}dk`;
  openM("questOverlay");
}
...
function claimGift(i){
  ...
  S.set(me,d);updateUI();notif("🎁 Hediye alındı!");openQuests();
}
...
function claimQ(qid){
  ...
  q.claimed=true;
  d.coins=Math.floor(d.coins+qd.reward);
  d.qDone=(d.qDone||0)+1;
  d.qs.coinsNow=Math.floor(d.coins);
  S.set(me,d);chkBadges(d);updateUI();notif(`🎉 +${qd.reward} coin!`);openQuests();
}
// ── SHOP ──
...
function renderShop(){
  ...
  document.getElementById("shopCoins").textContent=isOwner?"∞":Math.floor(d.coins);
  document.getElementById("shopGrid").innerHTML=SHOP[curCat].map(item=>{
    ...
    const speedT=item.speed?`<div class="sc-speed">⚡ ${item.speed}</div>`:"";
    const lockT=locked?`<div class="sc-req">🔒 Seviye ${item.minLvl}</div>`:"";
    const priceS=locked?`Lv.${item.minLvl}`:isOwner?"∞":`💰 ${item.price}`;
    let btnLabel=owned?"✓ Aktif":locked?"🔒 Kilitli":!afford?`❌ ${item.price} gerekli`:"Satın Al";
...
function buyFish(type){
  ...
  const ul=isOwner?99:d.level;
  if((item.minLvl||1)>ul)return notif(`🔒 Seviye ${item.minLvl} gerekli!`);
  if(!isOwner&&d.coins<item.price)return notif(`❌ Para yok! Gereken:${item.price} Sahip:${Math.floor(d.coins)}`);
  if(!isOwner)d.coins=Math.floor(d.coins-item.price);
...
function showLvlUp(lvl){
  ...
  el.innerHTML=`<h2 style="font-family:'Orbitron',monospace;font-size:11px;color:var(--accent2);letter-spacing:3px;margin-bottom:12px">⭐ SEVİYE ATLADIN!</h2>
...
function showBadge(b){
  ...
  el.innerHTML=`<h2 style="font-family:'Orbitron',monospace;font-size:11px;color:var(--gold);letter-spacing:3px;margin-bottom:12px">🎉 ROZET KAZANDIN!</h2>
...
function renderOwner(){
  ...
  if(!oUsers.length){el.innerHTML='<div style="color:rgba(200,240,255,.32);font-size:13px;text-align:center;padding:20px">Kayıtlı kullanıcı yok</div>';return;}
  el.innerHTML=oUsers.map(un=>{
    ...
    const top=`<div class="u-row-top"><img class="u-av" src="${av}" alt><div class="u-info"><div class="u-name">${un}</div><div class="u-stats">Lv.${lvl} · ${fish} balık · 💰${coins}</div></div></div>`;
    if(curOTab==="badge"){
      const manual=BADGES.filter(b=>!b.auto);
      return`<div class="u-row">${top}<select class="o-select" id="bs_${un}">${manual.map(b=>`<option value="${b.id}">${b.icon} ${b.name}</option>`).join("")}</select><button class="o-btn o-btn-gold" onclick="sendBadge('${un}')">🏅 Rozet Gönder</button></div>`;
    }
    if(curOTab==="coin")return`<div class="u-row">${top}<input class="o-input" id="ca_${un}" type="number" placeholder="Coin miktarı" min="1"><button class="o-btn o-btn-gold" onclick="sendCoin('${un}')">💰 Gönder</button></div>`;
    if(curOTab==="item")return`<div class="u-row">${top}<select class="o-select" id="is_${un}">${SHOP.special.map(s=>`<option value="${s.id}">${s.emoji} ${s.name}</option>`).join("")}</select><button class="o-btn o-btn-green" onclick="sendItem('${un}')">🎁 Item Gönder</button></div>`;
    return"";
  }).join("");
}
function sendBadge(un){
  ...
  if(d.badges.includes(bid))return notif("⚠️ Bu rozet zaten var!");
  const bdef=BADGES.find(x=>x.id===bid);
  if(!d.gifts)d.gifts=[];
  d.gifts.push({type:"badge",badgeId:bid,message:`🏅 <b>${bdef?.icon} ${bdef?.name}</b> rozeti hediye edildi!`});
  d.badges.push(bid);S.set(un,d);notif(`✅ ${un} kullanıcısına rozet gönderildi!`);
}
function sendCoin(un){
  const inp=document.getElementById(`ca_${un}`);if(!inp)return;
  const amount=parseInt(inp.value)||0;
  if(amount<=0)return notif("❌ Geçersiz miktar!");
  const d=S.get(un);if(!d)return;
  if(!d.gifts)d.gifts=[];
  d.gifts.push({type:"coin",amount,message:`💰 <b>${amount} coin</b> hediye edildi!`});
  d.coins=Math.floor((parseFloat(d.coins)||0)+amount);
  S.set(un,d);inp.value="";notif(`✅ ${un} kullanıcısına ${amount} coin gönderildi!`);
}
function sendItem(un){
  ...
  if((d.specials||[]).includes(itemId))return notif("⚠️ Bu item zaten var!");
  const idef=SHOP.special.find(x=>x.id===itemId);
  if(!d.gifts)d.gifts=[];
  d.gifts.push({type:"item",itemId,message:`${idef?.emoji} <b>${idef?.name}</b> özel eşyası hediye edildi!`});
  if(!d.specials)d.specials=[];d.specials.push(itemId);
  S.set(un,d);notif(`✅ ${un} kullanıcısına ${idef?.name} gönderildi!`);
}
...
function updateUI(){
  ...
  document.getElementById("tCoins").textContent=isOwner?"∞":Math.floor(d.coins);
  ...
  else{document.getElementById("xpBar").style.width="100%";document.getElementById("xpLabel").textContent="MAX 🔱";}
  document.getElementById("sFishCount").textContent=d.fish.length;
  const fl=document.getElementById("sFishList");
  fl.innerHTML=d.fish.length===0?'<div class="no-content">Henüz balık yok</div>':
  ...
  be.innerHTML=bs.length===0?'<div class="no-content">Henüz rozet yok</div>':
